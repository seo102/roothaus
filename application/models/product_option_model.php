...
class Product_option_model extends CI_Model {
	...
	function get_product_option_list_by_type($optionTypeId, $estimateId)
	{
		/*
		...
		*/
		...
		$sql = "select a.*, b.product_option_id as 'selected_option' from est_product_option a left join est_estimate_product b 
				on a.option_id = b.product_option_id and b.estimate_id = '".$this->db->escape_str($estimateId)."'
				where a.option_type_id = '".$this->db->escape_str($optionTypeId)."' order by a.option_id";
