...
class Estimate_product_model extends CI_Model {
	...
	function insert_estimate_product($categoryId, $estimateId, $productId, $productOptionId)
	{
		$sql = "insert into est_estimate_product set category_id = '".$this->db->escape_str($categoryId)."', estimate_id = '".$this->db->escape_str($estimateId)."', product_id = '".$this->db->escape_str($productId)."', product_option_id = '".$this->db->escape_str($productOptionId)."'";	
	...
	function update_estimate_product($categoryId, $estimateId, $productId, $productOptionId, $inputArray)
	{				
		$updateArray = array(			
			"product_option_id" => $inputArray->post('product_option_id', true)
		);
		$whereArray = array (
